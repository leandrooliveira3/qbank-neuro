...
const questionSchema = {
  ...
  items: {
    ...
    properties: {
      ...
      comentario: { type: Type.STRING, description: "Justificativa clínica profunda escrita por um preceptor médico" },
      dificuldade: { type: Type.STRING },
      tags: { type: Type.ARRAY, items: { type: Type.STRING } }
    },
    required: ["categoria", "enunciado", "alternativas", "gabarito", "comentario", "dificuldade"],
  },
};

const SYSTEM_INSTRUCTION = `Você é um Professor Titular de Medicina e Preceptor Sênior. 
Sua missão é projetar questões de alto nível técnico para um Banco de Questões Inteligente.

REGRA DE FORMATO CRÍTICA:
- O campo 'alternativas' deve conter APENAS o texto das opções.
- NUNCA inclua prefixos como "A) ", "B. ", "1 - " ou similares.`;
